...
do
    ...
    do
        ...
        course_name=`egrep "\*+.*$course_code.*" handbook_entry.txt.tmp | sed -e 's/\*//g' | sed -E 's/^ (.*) - .*/\"\1\"/g'`
        # wrap in quotes
        course_code=`echo $course_code | sed -E 's/(.*)/\"\1\"/g'`
        # wrap in quotes
        handbook_url=`echo "$handbook_url" | sed -E 's/(.*)/\"\1\"/g'`
        echo "{\"code\":$course_code, \"name\":$course_name, \"faculty\":$faculty, \"co_reqs\":$co_reqs, \"pre_reqs\":$pre_reqs, \"exclusions\":$excluded, \"handbook_url\":$handbook_url}"
    done
done
